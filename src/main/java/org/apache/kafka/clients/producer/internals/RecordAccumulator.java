...
public final class RecordAccumulator {
    ...
    private final ConcurrentMap<TopicPartition, DequeLock<ProducerBatch>> batches;
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            DequeLock<ProducerBatch> dq = getOrCreateDeque(tp);
            dq.lock.lock();
            try {
                if (closed)
    ...
    public List<ProducerBatch> expiredBatches(long now) {
        ...
        for (Map.Entry<TopicPartition, DequeLock<ProducerBatch>> entry : this.batches.entrySet()) {
            // expire the batches in the order of sending
            DequeLock<ProducerBatch> deque = entry.getValue();
            deque.lock.lock();
            try {
                while (!deque.isEmpty()) {
    ...
    public void reenqueue(ProducerBatch batch, long now) {
        ...
        DequeLock<ProducerBatch> deque = getOrCreateDeque(batch.topicPartition);
        deque.lock.lock();
        try {
    ...
    public int splitAndReenqueue(ProducerBatch bigBatch) {
        ...
        DequeLock<ProducerBatch> partitionDequeue = getOrCreateDeque(bigBatch.topicPartition);
        while (!dq.isEmpty()) {
            ProducerBatch batch = dq.pollLast();
            incomplete.add(batch);
            // We treat the newly split batches as if they are not even tried.
            partitionDequeue.lock.lock();
            try {
                if (transactionManager != null) {
    ...
    private List<ProducerBatch> drainBatchesForOneNode(Cluster cluster, Node node, int maxSize, long now) {
        ...
        do {
            ...
            DequeLock<ProducerBatch> deque = getDeque(tp);
            if (deque == null)
                continue;
    ...
    private DequeLock<ProducerBatch> getDeque(TopicPartition tp) {
     ...
     */
    private DequeLock<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        DequeLock<ProducerBatch> d = this.batches.get(tp);
        if (d != null)
            return d;
        d = new DequeLock<>();
        DequeLock<ProducerBatch> previous = this.batches.putIfAbsent(tp, d);
        if (previous == null)
            return d;
        else
            return previous;
    ...
    void abortBatches(final RuntimeException reason) {
        for (ProducerBatch batch : incomplete.copyAll()) {
            DequeLock<ProducerBatch> dq = getDeque(batch.topicPartition);
            dq.lock.lock();
            try {
                batch.abortRecordAppends();
                dq.remove(batch);
    ...
    private final static class DequeLock<E> extends ArrayDeque<E> {
