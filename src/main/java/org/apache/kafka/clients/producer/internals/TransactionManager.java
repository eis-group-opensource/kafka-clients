...
public class TransactionManager {
    ...
    TransactionalRequestResult initializeTransactions(ProducerIdAndEpoch producerIdAndEpoch) {
        lock.lock();
        try {
    ...
    public TransactionalRequestResult sendOffsetsToTransaction(final Map<TopicPartition, OffsetAndMetadata> offsets,
    ...
    public void maybeAddPartitionToTransaction(TopicPartition topicPartition) {
        lock.lock();
        try {
    ...
    public void failIfNotReadyForSend() {
        lock.lock();
        try {
            if (hasError())
    ...
    boolean isSendToPartitionAllowed(TopicPartition tp) {
        lock.lock();
        try {
            if (hasFatalError())
                return false;
    ...
    void transitionToFatalError(RuntimeException exception) {
        lock.lock();
        try {
    ...
    // visible for testing
    boolean isPartitionAdded(TopicPartition partition) {
        lock.lock();
        try {
    ...
    // visible for testing
    boolean isPartitionPendingAdd(TopicPartition partition) {
        lock.lock();
        try {
    ...
    void requestEpochBumpForPartition(TopicPartition tp) {
        lock.lock();
        try {
    ...
    void bumpIdempotentEpochAndResetIdIfNeeded() {
        lock.lock();
        try {
            if (!isTransactional()) {
                if (shouldBumpEpoch()) {
     ...
     */
    int firstInFlightSequence(TopicPartition topicPartition) {
        lock.lock();
        try {
    ...
    public void handleCompletedBatch(ProducerBatch batch, ProduceResponse.PartitionResponse response) {
        lock.lock();
        try {
    ...
    void handleFailedBatch(ProducerBatch batch, RuntimeException exception, boolean adjustSequenceNumbers) {
        lock.lock();
        try {
    ...
    boolean hasUnresolvedSequences() {
        lock.lock();
        try {
    ...
    boolean hasUnresolvedSequence(TopicPartition topicPartition) {
        lock.lock();
        try {
    ...
    void markSequenceUnresolved(ProducerBatch batch) {
        lock.lock();
        try {
    ...
    TxnRequestHandler nextRequest(boolean hasIncompleteBatches) {
        lock.lock();
        try {
            if (!newPartitionsInTransaction.isEmpty())
                enqueueRequest(addPartitionsToTransactionHandler());
    ...
    void authenticationFailed(AuthenticationException e) {
        lock.lock();
        try {
    ...
    boolean canRetry(ProduceResponse.PartitionResponse response, ProducerBatch batch) {
        lock.lock();
        try {
