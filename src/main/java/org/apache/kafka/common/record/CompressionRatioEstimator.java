...
public class CompressionRatioEstimator {
    ...
    private static final ConcurrentMap<String, Ratio> COMPRESSION_RATIO = new ConcurrentHashMap<>();
    ...
    public static float updateEstimation(String topic, CompressionType type, float observedRatio) {
        Ratio compressionRatioForTopic = getAndCreateEstimationIfAbsent(topic);
        float currentEstimation = compressionRatioForTopic.value[type.id];
        compressionRatioForTopic.lock.lock();
        try {
            if (observedRatio > currentEstimation)
                compressionRatioForTopic.value[type.id] = Math.max(currentEstimation + COMPRESSION_RATIO_DETERIORATE_STEP, observedRatio);
            else if (observedRatio < currentEstimation) {
                compressionRatioForTopic.value[type.id] = Math.max(currentEstimation - COMPRESSION_RATIO_IMPROVING_STEP, observedRatio);
            }
        } finally {
            compressionRatioForTopic.lock.unlock();
        }
        return compressionRatioForTopic.value[type.id];
    }
    ...
    public static void resetEstimation(String topic) {
        ...
        try {
            for (CompressionType type : CompressionType.values()) {
                compressionRatioForTopic.value[type.id] = type.rate;
            }
        } finally {
            compressionRatioForTopic.lock.unlock();
        }
    ...
    public static void setEstimation(String topic, CompressionType type, float ratio) {
        ...
        compressionRatioForTopic.lock.lock();
        try {
            compressionRatioForTopic.value[type.id] = ratio;
        } finally {
            compressionRatioForTopic.lock.unlock();
        }
    }

    private static Ratio getAndCreateEstimationIfAbsent(String topic) {
        Ratio compressionRatioForTopic = COMPRESSION_RATIO.get(topic);
        if (compressionRatioForTopic == null) {
            compressionRatioForTopic = initialCompressionRatio();
            Ratio existingCompressionRatio = COMPRESSION_RATIO.putIfAbsent(topic, compressionRatioForTopic);
