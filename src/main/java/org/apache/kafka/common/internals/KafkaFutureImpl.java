...
public class KafkaFutureImpl<T> extends KafkaFuture<T> {
    ...
    private static class SingleWaiter<R> implements BiConsumer<R, Throwable> {
        ...
        @Override
        public void accept(R newValue, Throwable newException) {
            lock.lock();
            try {
                this.value = newValue;
        ...
        R await() throws InterruptedException, ExecutionException {
            lock.lock();
            try {
                while (true) {
                    if (exception != null)
        ...
        R await(long timeout, TimeUnit unit)
            ...
            try {
                while (true) {
                    ...
                    condition.await(waitTimeMs - delta, TimeUnit.MILLISECONDS);
    ...
    protected void addWaiter(BiConsumer<? super T, ? super Throwable> action) {
        lock.lock();
        try {
            if (exception != null) {
